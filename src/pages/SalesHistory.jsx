...
export default function SalesHistory() {
  ...
  function loadData() {
    getSales().then((d) => setSales(d)).catch(console.error).finally(() => setLoading(false));
  }
  ...
  const totals = useMemo(() => {
    ...
    filtered.forEach((sale) => {
      ...
      if (status === "cancelled") {
        cancelled++;
        return; // Exclude from revenue and units
      }
  ...
  async function handleConfirmCancel(reason) {
    ...
    setLoading(true);
    try {
      await cancelSale({ saleId: cancelSaleTarget.id, reason, user });
      loadData();
    } catch (err) {
  ...
  return (
    <>
      ...
      {returnSaleTarget && <ReturnSaleModal sale={returnSaleTarget} onClose={() => setReturnSaleTarget(null)} onSuccess={() => { setReturnSaleTarget(null); loadData(); }} />}
      {cancelSaleTarget && (
        <ConfirmModal
          state={{
            isOpen: true,
            title: `Cancel Invoice ${cancelSaleTarget.invoiceNumber}`,
            desc: "Are you sure you want to completely cancel this sale? This will restore inventory quantities. This action cannot be undone.",
              ...
              <table className="w-full text-sm text-left text-white/80 min-w-[900px]">
                ...
                <tbody>
                  {loading ? (
                    ...
                    return (
                      <tr key={sale.id} className={`border-t border-white/10 hover:bg-white/[0.02] transition ${isCancelled ? "opacity-60" : ""}`}>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2 justify-end">
                            {status === "completed" && (
                              <button onClick={() => setCancelSaleTarget(sale)} className="rounded-lg bg-red-500/10 ring-1 ring-red-500/20 text-red-300 text-xs font-semibold px-2.5 py-1.5 hover:bg-red-500/20 transition">Cancel</button>
                            )}
                            {(status === "completed" || status === "partially_returned") && (
                              <button onClick={() => setReturnSaleTarget(sale)} className="rounded-lg bg-amber-500/10 ring-1 ring-amber-500/20 text-amber-300 text-xs font-semibold px-2.5 py-1.5 hover:bg-amber-500/20 transition">Return</button>
                            )}
                            <button onClick={() => setInvoiceSale(sale)} className="inline-flex items-center gap-1 rounded-lg bg-blue-500/15 ring-1 ring-blue-500/25 text-blue-200 text-xs font-semibold px-2.5 py-1.5 hover:bg-blue-500/25 transition">🧾 Invoice</button>
                          </div>
                        </td>
                      </tr>
                    );
